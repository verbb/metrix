...
export const PieWidget = (props) => {
    ...
    const fetchData = useCallback((payload) => {
        return api.get('widget-data', { data: 'pie-widget-data', ...payload });
    }, []);
    ...
    function preprocessData(rows, thresholdPercentage = 1) {
        ...
        if (otherValue > 0) {
            groupedRows.push(['Other', otherValue]); // Add "Other" category
        }
    ...
    function renderContent(data) {
        ...
        const processedRows = preprocessData(data.chart.rows, 1); // Group values < 1%
    ...
    return <WidgetLarge className="mc-h-[29rem]" fetchData={fetchData} afterFetchData={afterFetchData} renderContent={renderContent} {...props} />;
};
