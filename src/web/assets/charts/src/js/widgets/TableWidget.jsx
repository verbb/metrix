...
export const TableWidget = (props) => {
    ...
    const fetchData = useCallback((payload) => {
        return api.get('widget-data', { data: 'table-widget-data', ...payload });
    }, []);

    function renderContent(data) {
        const totalPages = Math.ceil(data.chart.rows.length / MAX_ITEMS);
        const hasPagination = data.chart.rows.length > MAX_ITEMS;
        ...
        function sortRows(rows) {
            ...
            const colIndex = data.chart.cols.findIndex((col) => {
                return col.id === sortConfig.key;
            });

            const colType = data.chart.cols[colIndex]?.type;
        ...
        function getPaginatedRows() {
            const sortedRows = sortRows(data.chart.rows);
        ...
        function Bar({ row, colIndex, children }) {
            const width = barWidth(row[colIndex], colIndex, data.chart.rows);
