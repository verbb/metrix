...
final class MetrixTestPlugin
{
    public static function ensureInstalledAndMigrated(): void
    {
        ...
        if (!$plugins->isPluginInstalled('metrix')) {
            $plugins->installPlugin('metrix');
        } elseif (!$plugins->isPluginEnabled('metrix')) {
            $plugins->enablePlugin('metrix');
        }

        $plugin = $plugins->getPlugin('metrix');
        if (!$plugin) {
            throw new \RuntimeException('Metrix plugin failed to load for integration tests.');
        }
